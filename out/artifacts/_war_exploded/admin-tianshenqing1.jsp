	        		...
	        		<div class="content-main" style="position: relative;">
	        			<div style="padding-bottom: 10px;padding-top: 20px;">
							<%
								...
								ed.openConn();
								int totalpages=ed.getTotalPage(pageSize,"Table_draftinfo where state = '推荐中'"); //最大页数
								...
								List list= ed.getDraft_ApprovalAllempByPage(pageSize,pageIndex,"推荐中");  //返回特定页数的数据
								ed.conn.close();
							%>
	        			<table  class="layui-table" style="width: 700px;margin:0 auto;">
							...
							<tr>
								...
								<td><%=map.get("ID") %></td>
								<td><a href="admin-content1.jsp?o=7&id=<%=map.get("ID")%>"><%=map.get("name") %></a></td>
				                ...
				                <td colspan="1">
				                    <div align="center">
				                        <div align="right">
											...
											<a href="admin-tianshenqing1.jsp?o=7&pageIndex=1">首页</a>
											<a href="admin-tianshenqing1.jsp?o=7&pageIndex=<%=pageIndex-1 %>">上一页</a>
											<a href="admin-tianshenqing1.jsp?o=7&pageIndex=<%=pageIndex+1 %>">下一页</a>
											<a href="admin-tianshenqing1.jsp?o=7&pageIndex=<%=totalpages%>">尾页</a>
				                        </div>
				                    </div>
				                </td>
				            </tr>
	        			...
	        			<div style="padding-left: 68px;">
	        				<a id="pass" href="#" class="layui-btn" style="width: 120px;">推荐</a>
        ...
        $('input[type="checkbox"]').each(function(index){
            if(index!=0){
                if($('input[type="checkbox"]').eq(index).prop('checked')){
                    ...
                    var k=$p.html();
                    $.post("server.jsp?action=qing1-1",{id:k},function (res) {
                        if("ok"==res){
...
//                             alert(index);
                    $p=$('input[type="checkbox"]').eq(index).parent().next("td");
                    var k=$p.html();
                    $.post("server.jsp?action=qing1-2",{id:k},function (res) {
                        if("ok"==res){
                            $('input[type="checkbox"]').each(function(index1){
                                if($('input[type="checkbox"]').eq(index1).parent().next('td').html()==k){
                                    $('input[type="checkbox"]').eq(index1).parent().parent().remove();
