...
public class FormConnector {
    ...
    public List getUsersinfoAllempByPage(int pageSize,int pageIndex,String name){
        ...
        else
        {
            ...
            String S = String.valueOf(pageSize);
            sql = "select ID,name from (Table_users1 join Table_usersinfo on Table_users1.username = Table_usersinfo.username) where name = " + "'" + name + "'" + " and  status = '待审核' order by ID asc ";
        }
            try {
                psmt = conn.prepareStatement(sql);
                rs = psmt.executeQuery();
